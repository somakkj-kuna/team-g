...
PY_SCRIPT="${SCRIPT_DIR}/libs/nifs_collect.py"
CONFIG_FILE="${SCRIPT_DIR}/config/nifs_config.toml"
CONDA_BIN="/home/collect/appl/miniconda3/bin/conda"

usage() {
  cat <<'USAGE'
Usage:
  collect_nifs_obs.sh [START_YYYYMMDD] [END_YYYYMMDD] [--show-sample] [--limit N]
                      [--areas 동해,남해,서해] [--station-codes CODE1,CODE2]

Examples:
  collect_nifs_obs.sh
  collect_nifs_obs.sh 20260330 --show-sample --limit 3
  collect_nifs_obs.sh 20260330 20260331
...
if [[ "${start_date}" > "${end_date}" ]]; then
  echo "[ERROR] start date must be <= end date: ${start_date} > ${end_date}" >&2
  exit 1
fi
